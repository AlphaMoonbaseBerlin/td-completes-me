'''Info Header Start
Name : td_completes_me_ext
Author : Wieland@AMB-ZEPH15
Saveorigin : test_bed.toe
...
class TDCompletesMe :
	def __init__(self, ownerComp) :
		...
		self._msg_data = None
		if self.ownerComp.par.Autosetup.eval(): self.Setup()
	...
	def Setup(self):
		...
		self.ownerComp.par.Port.val = self.ownerComp.op("amb_python_utils").Get_Free_Network_Port()
		os.environ["TD_COMPLETESME_PORT"] = str( self.ownerComp.par.Port.eval() )
		run( "args[0]()", self._Setup, delayFrames = 4)


	def _Setup(self):
		self.ownerComp.par.Active.val = True
